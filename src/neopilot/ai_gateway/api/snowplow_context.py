...
def get_snowplow_code_suggestion_context(
    ...
    if req.user and req.user.claims and req.user.claims.issuer == CloudConnectorConfig().service_name:
    ...
    return SnowplowEventContext(
        ...
        gitlab_saas_neoai_pro_namespace_ids=list(
            map(
                int,
                CommaSeparatedStrings(req.headers.get(X_GITLAB_SAAS_NEOAI_PRO_NAMESPACE_IDS_HEADER, "")),
            )
        ),
        language=language,

...
class EditorContentPayload(BaseModel):
    ...
    file_name: Annotated[str, StringConstraints(strip_whitespace=True, max_length=255)] = Field(examples=["example.py"])
    content_above_cursor: Annotated[str, StringConstraints(max_length=100000)] = Field(
        examples=["def hello_world():\n    print("]
    )
    content_below_cursor: Annotated[str, StringConstraints(max_length=100000)] = Field(examples=[""])
    language_identifier: Optional[Annotated[str, StringConstraints(max_length=255)]] = Field(None, examples=["python"])
    model_provider: Optional[Literal[ModelProvider.VERTEX_AI, ModelProvider.ANTHROPIC, KindModelProvider.AMAZON_Q]] = (
        None
    )
    model_name: Optional[str] = Field(None, examples=[KindVertexTextModel.CODE_GECKO_002])
...
class EditorContentCompletionPayload(EditorContentPayload):
    ...
    prompt: Optional[str | list[Message]] = Field(None, examples=["Complete the function"])
...
class CompletionRequest(BaseModel):
    prompt_components: Annotated[List[PromptComponent], Field(min_length=1, max_length=100)]

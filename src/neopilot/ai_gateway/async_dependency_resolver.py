...
See https://github.com/neopilot-ai/neopilot/-/merge_requests/606 for more
information.
"""
...
async def get_anthropic_proxy_client(
    anthropic_proxy_client=Provide[ContainerApplication.pkg_models.anthropic_proxy_client],
):
...
async def get_vertex_ai_proxy_client(
    vertex_ai_proxy_client=Provide[ContainerApplication.pkg_models.vertex_ai_proxy_client],
):
...
async def get_amazon_q_client_factory(
    amazon_q_client_factory=Provide[ContainerApplication.integrations.amazon_q_client_factory],
):

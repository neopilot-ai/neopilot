...
from neopilot.ai_gateway.auth.gcp import access_token
...
class VertexAIProxyClient(BaseProxyClient):
    ...
    def _extract_upstream_path(self, request_path: str) -> str:
        ...
        return (
            f"/v1/projects/{self.project}/locations/{self.location}/publishers/google/models/{model}:{action}{sse_flag}"
        )

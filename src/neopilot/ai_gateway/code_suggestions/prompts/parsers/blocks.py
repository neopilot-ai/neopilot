...
from neopilot.ai_gateway.code_suggestions.prompts.parsers.base import BaseVisitor, Point
...
class ErrorBlocksVisitor(BaseVisitor):
    ...
    def _visit_node(self, target_node: Node):
        ...
        for node in self.error_nodes:
            if node.start_point <= target_node.start_point and target_node.end_point <= node.end_point:
                continue
            nodes.append(node)

...
from neopilot.ai_gateway.code_suggestions.processing.ops import LanguageId
from neopilot.ai_gateway.code_suggestions.prompts.parsers.base import BaseVisitor
...
class BaseContextVisitor(BaseVisitor):
    ...
    def visit(self, node: Node):
        ...
        if self._is_point_within_rectangle(self.target_point, node.start_point, node.end_point):
    ...
    def extract_most_relevant_context(self, priority_list: Optional[List[str]] = None) -> Optional[Node]:
        if priority_list:
    ...
    def _make_priority_map(self, importance_list):
        return {symbol: priority for priority, symbol in enumerate(reversed(importance_list))}

...
_ALL_LANGS = {
    ...
    _LanguageDef(LanguageId.CSHARP, "c_sharp", "C#", frozenset({"cs"}), frozenset({"csharp"})),
    ...
    _LanguageDef(LanguageId.PYTHON, "python", "Python", frozenset({"py"}), frozenset({"python"})),
    _LanguageDef(LanguageId.RUBY, "ruby", "Ruby", frozenset({"rb"}), frozenset({"ruby"})),
    _LanguageDef(LanguageId.RUST, "rust", "Rust", frozenset({"rs"}), frozenset({"rust"})),
    _LanguageDef(LanguageId.SCALA, "scala", "Scala", frozenset({"scala"}), frozenset({"scala"})),
    _LanguageDef(
        LanguageId.TS,
        "typescript",
        "TypeScript",
...
_EXTENSION_TO_LANG_ID = {ext: language.lang_id for language in _ALL_LANGS for ext in language.extensions}

_EDITOR_LANG_TO_LANG_ID = {name: language.lang_id for language in _ALL_LANGS for name in language.editor_names}

_EXTENSION_TO_LANG_NAME = {ext: language.grammar_name for language in _ALL_LANGS for ext in language.extensions}
...
def remove_incomplete_block(s: str, max_trim_percent: float = _MAX_CODE_BLOCK_TRIM_PERCENT) -> str:
    end_of_block = _END_OF_CODE_BLOCK_REGEX.search(s, endpos=int(len(s) * max_trim_percent))
    if end_of_block:
    ...
    for i in range(len_source + 1):
        for j in range(len_target + 1):
            if i == 0 or j == 0:
                ...
                l_matrix[i - 1, j - 1] = prev_match + 1 if prev_match == 1 else prev_match
...
def split_on_point(source_code: str, point: tuple[int, int]) -> tuple[Optional[str], Optional[str]]:
...
def convert_point_to_relative_point_in_node(node: Node, point: tuple[int, int]) -> tuple[int, int]:

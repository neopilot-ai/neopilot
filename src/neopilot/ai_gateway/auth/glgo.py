...
cloud_connector_token_context_var: contextvars.ContextVar = contextvars.ContextVar("cloud_connector_token")
...
class GlgoAuthority:
    ...
    def _build_kid(self, signing_key):
        jwk_dict = jwk.RSAKey(algorithm=self.ALGORITHM, key=signing_key).public_key().to_dict()
        normalized_data = {key: jwk_dict[key] for key in ["e", "kty", "n"] if key in jwk_dict}
        serialized_data = json.dumps(normalized_data, separators=(",", ":"), sort_keys=True).encode()

...
class Flow(AbstractWorkflow):
    ...
    def _process_additional_context(self, additional_context: list[AdditionalContext]) -> Dict:
        ...
        for item in additional_context:
            if item.category == "os_information":  # This category is passed in from the executor
    ...
    def _compile(
        ...
        if self._config.flow.entry_point is None:
            raise ValueError("Can not compile flow: entry_point is not defined in the flow config.")
